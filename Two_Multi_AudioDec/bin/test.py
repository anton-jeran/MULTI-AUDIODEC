    ...
    def run(self):
        ...
        with torch.no_grad(), tqdm(self.dataset, desc="[test]") as pbar:
            for idx, (utt_id, x) in enumerate(pbar, 1):
                ...
                y_reverb_speech_0 = cp.asnumpy(fftconvolve(y_speech_cpu[:,0:1],y_rir_cpu[:,0:2],axes=0)[0:y_speech.shape[0],:])
                y_reverb_speech_1 = cp.asnumpy(fftconvolve(y_speech_cpu[:,1:2],y_rir_cpu[:,2:4],axes=0)[0:y_speech.shape[0],:])
