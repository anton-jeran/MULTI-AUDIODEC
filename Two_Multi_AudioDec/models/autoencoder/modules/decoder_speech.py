...
class Decoder_SPEECH(torch.nn.Module):
    def __init__(self,
        code_dim, 
        output_channels,
        decode_channels,
        ...
        self.gen_masks = Conv1d((decode_channels * channel_ratios[0]), (decode_channels * channel_ratios[0])*2, 1, bias=False)
    ...
    def forward(self, z):
        x = self.conv1(z)
        x1 = self.gen_masks(x)
        x2 = torch.chunk(x1, chunks=2, dim=1)
        # print("x2 shape ", x2.shape)
        x2 = self.activation(torch.stack(x2, dim=0))
        ...
        for i in range(self.num_blocks):
            spk1 = self.conv_blocks[i](spk1)
        spk1 = self.conv2(spk1)

        for i in range(self.num_blocks):
            spk2 = self.conv_blocks[i](spk2)
        spk2 = self.conv2(spk2)
        
        
        speech = torch.cat((spk1,spk2),1)
