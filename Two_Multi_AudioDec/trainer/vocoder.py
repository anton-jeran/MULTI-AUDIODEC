...
class Trainer(TrainerGAN):
    ...
    def _train_step(self, batch):
        ...
        if self.steps > self.generator_start:
            ...
            # metric loss
            gen_loss += self._metric_loss_speech(y_speech_, cs, 300000,mode=mode)
            gen_loss += self._metric_loss_reverb_speech(y_reverb_speech_, rs,  300000,mode=mode)
            gen_loss += self._metric_loss_rir(y_rir_, rir, self.filters, 300000,mode=mode)
