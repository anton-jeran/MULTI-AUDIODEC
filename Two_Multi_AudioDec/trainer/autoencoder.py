...
class Trainer(TrainerVQGAN):
    ...
    def _train_step(self, batch):
        ...
        if self.generator_train:
            ...
            y_reverb_speech_0 = torch.tensor(fftconvolve(y_speech_cpu[:,0:1,:],y_rir_cpu[:,0:2,:],axes=2)[:,:,0:y_speech_.shape[2]],device='cuda')
            y_reverb_speech_1 = torch.tensor(fftconvolve(y_speech_cpu[:,1:2,:],y_rir_cpu[:,2:4,:],axes=2)[:,:,0:y_speech_.shape[2]],device='cuda')
            y_reverb_speech_ =  (y_reverb_speech_0 + y_reverb_speech_1) /2
        ...
        if(self.steps%5000==0):
            ...
            for i in range(rir.shape[0]):
                ...
                r = WaveWriter(real_RIR_path, channels=4, samplerate=fs)
                r.write(np.array(real_IR))
                f = WaveWriter(fake_RIR_path, channels=4, samplerate=fs)
            ...
            for i in range(cs.shape[0]):
                ...
                r = WaveWriter(real_SPEECH_path, channels=2, samplerate=fs)
                r.write(np.array(real_SPEECH))
                f = WaveWriter(fake_SPEECH_path, channels=2, samplerate=fs)
